...
void dfs(const unordered_map<string, vector<string>>& graph, const string& city, const string& start, unordered_map<string, string>& countryMap, unordered_map<string, string>& pathMap, unordered_map<string, int>& distanceMap) {
    for (const string& neighbor : graph.at(city)) {
        if (countryMap.find(neighbor) == countryMap.end()) {
            ...
            dfs(graph, neighbor, start, countryMap, pathMap, distanceMap);
        }
    }
}

void findShortestPath(const unordered_map<string, vector<string>>& graph, const string& start) {
    ...
    dfs(graph, start, start, countryMap, pathMap, distanceMap);
